    ...
    static private class game{
        ...
        private class game_main{
            ...
            private void initialize(String[] location, int[] total, unit[] edits, ArrayList<String> players){
                ...
                while (!game.this.players.parallelStream().filter(i -> i.equals(empty_builder.toString())).findAny().isEmpty()) { 
                    if(empty_builder.charAt(empty_builder.length()-1) == EMPTY_BUIDER[EMPTY_BUIDER.length-1]){
                        empty_builder.append(EMPTY_BUIDER[0]);
                    }else{
                        empty_builder.replace(empty_builder.length()-1, empty_builder.length(), EMPTY_BUIDER[empty_builder.c]);
                    }
                }
        ...
        @SuppressWarnings("ImplicitArrayToString")
        private int[] digital_to_array_transforming(Supplier<Integer> digitals,int locations) throws Exception{
            //if (choosen_digitals.length != numbers.length) {throw new Exception("The number of choosen digitals is not equals to \"numbers\".");}
            //if (digitals.length != locations.length) {throw new Exception("The number of digitals is not equals to locations'.");}
            int table = 1;
