...
public class tic_tac_toc {
    ...
    private static final class game{
        logger.file_logger log() /*throws Exception*/{
            Path root = Paths.get("logs");
            String game_id = logger.file_id_creater.next_id();
            logger.file_logger logger_in_log = new logger.file_logger(root, game_id, true, "game");
        ...
        game_show game_show;
        logger.file_logger logger_in_game;
        kit_with_logger kit = new kit_with_logger();
        ...
        private void initialize(String[] location, int[] total, unit[] edits, ArrayList<String> players){
            ...
            try {
                ...
                dne.add(empty);
                DNE = kit.get_a_unused_string(dne, EMPTY_BUIDER);
            } catch (Exception e) {
        ...
        private void initialize(int[] total, unit[] edits){
            initialize((String[])directions_map.keySet().toArray(), total, edits, players);
        }
        private void initialize(int[] total/* , unit[] edits */){
            //initialize(/* (String[])directions_map.keySet().toArray() */null, total, /* edits */null, players);
            total_numbers_of_directions = total;
            try {
                empty = "";
                ArrayList<String> dne = new ArrayList<>(game.this.players);
    ...
    //    }
        class show extends Frame{
            ...
            protected void reflash(){
                ...
                String x_key = null,y_key = null;
                try{
                    x_key = kit.get_unique_key(directions_map,directions_of_x);
                    y_key = kit.get_unique_key(directions_map, directions_of_y);
                }catch(Exception e){
                ...
                if(x_key == null || y_key == null){
                    String message = (x_key == null && y_key == null) ? "Both directions are" : "One of the directions is";
                    Dialog d = new Dialog(this,format("%s not found.", message),true);
                    d.add(new Label(format("%s not found.", message)));
                    d.addWindowListener(new WindowAdapter() {
                        @Override
                        public void windowClosing(WindowEvent e) {
                            d.setVisible(false);
                ...
                if(/*x_key[0] == y_key[0]*/directions_of_x == directions_of_y){
                    ...
                    //d.setVisible(true);
                    //return;
                    show_directions.setText("x,y:" + x_key);
                    button_board = new Component[1][total_numbers_of_directions[directions_of_x]];
                    for (int i = 0; i < button_board[0].length; i++) {
                        try {
                            final int DT = kit.mixed_to_decimal_adjusting_locations(total_numbers_of_directions,new int[]{directions_of_x},new int[]{i},locations);
                    ...
                    show_directions.setText(format("x:%s,y:%s",x_key,y_key));
                    ...
                    for (int i = 0;i<button_board.length;i++) {
                        for (int j = 0;j<button_board[i].length;j++){
                            //int[] ij = {i,j};
                            try {
                                int DT = kit.mixed_to_decimal_adjusting_locations(total_numbers_of_directions,/*directions*/new int[]{directions_of_x, directions_of_y},/*ij*/new int[]{i, j},locations);
                                final int fi = i, fj = j;
                ...
                Choice choose_direction = new Choice();
                {
                    send.addActionListener((_) -> {
                        ...
                        try {
                            ...
                            !exist_in_Array(units, kit.mixed_to_decimal_adjusting_locations(total_numbers_of_directions, 
                                                                        new int[]{directions_map.get(choose_direction.getSelectedItem())}, 
                                                                        new int[]{par}, 
                                                                        locations)))
                            {return;}
                            ...
                            reflash();
                        //}catch(Exception e){}
                        }catch(NumberFormatException e){/*return;*/logger_in_game.log("log",e.getMessage());} catch (Exception e) {
                            // TODO Auto-generated catch block
                            show_exception(e, game_show);
                            //return;
                        }
                    });
                    for (String i : directions_map.keySet()) {
            ...
            protected void show_tip(MouseEvent e,int DT){
                try{
                    text.setText(Arrays.toString(kit.decimal_to_mixed(total_numbers_of_directions, DT)));
                    window.removeAll();
                    window.add(text);
                    window.pack();
        ...
        private class unit{
            ...
            @SuppressWarnings("unused")
            public Button set_button(Button button){
                this.button = button;
                return button;
            }
        }
        ...
        private class edit extends show{
            ...
            private final setting_in_edit setting_in_edit = new setting_in_edit();
            @Override
            protected void action_after_reflash(game game){
                for (int i = 0; i < button_board.length; i++) {
                    for (int j = 0; j < button_board[i].length; j++) {
                ...
                //if(!first){/* settings.addActionListener(_ -> {setting_in_edit.setVisible(true);}); */}//else{reflash();}
                if(!first){panel2.add(settings);}
                action_after_reflash_in_edit(game);
                this.pack();
            }
            protected void action_after_reflash_in_edit(@SuppressWarnings("unused") tic_tac_toc.game game) {}/*/ TODO Auto-generated method stub/*throw new UnsupportedOperationException("Unimplemented method 'action_after_reflash_in_edit'"); */
            @Override
            ...
            protected class setting_in_edit extends deraction{
                public setting_in_edit(){
                    done.addActionListener(_ -> {
                        setting_in_edit.this.setVisible(false);
                    });
                }
            }
            @SuppressWarnings("OverridableMethodCallInConstructor")
            public edit(int x, int y/* , int number */){
                super(x, y/* , number */);
                settings.addActionListener(_ -> {settings.setVisible(true);});
                for (String player : players) {
                    ...
                    i.addActionListener((_) -> {
                        ...
                        units[kit.mixed_to_decimal_adjusting_locations(total_numbers_of_directions, 
                                                   d, 
                                                   c, 
                ...
                pack();
                //System.out.println("000");
                settings.addActionListener(_ -> {setting_in_edit.setVisible(true);});
            }
        }
        ...
        private class creat_game extends edit{
            Button creat = new Button("creat");
            //int[] total;
            public creat_game(/* int number */int[] total){
                super(0, 0/* , number */);
                creat.addActionListener(_ -> {
                    game.this.initialize(total);
                });
            }
            public creat_game(/* int number *//* int[] total */){
                super(0, 0);
                creat.addActionListener(_ -> {
                    game.this.initialize(null);
                });
            }
        }
        private class setting extends deraction{
            ...
            public setting(){
                //done.setLabel("done");
                    ...
                    creat_game cg = new creat_game();
                    cg.setVisible(true);
                });
            }
        }
        //private class 
        private class deraction extends Frame{
            java.awt.List list = new java.awt.List();
            Button add = new Button("+");
            Button delete = new Button("-");
            Panel buttons = new Panel(new FlowLayout());
            //final String TEXT = "Creat";
            Button done = new Button(/* TEXT */"done");
            synchronous synchronous = new synchronous();
            public deraction(){
                //Map<String,Integer> map = new HashMap<>(/* directions_map */);
                //map.sort();
                //for(int i : directions_map.values()){
                    //list.add
                //}
                add.addActionListener(_ -> {
                    String input = input(/* this, */"creat a deraction","Please enter the name of deraction.");
                    if(input != null){
                        //list.add(input/* , list.getItemCount() */);
                        //boolean b = false;
                        int i = 0;
                        while(true){
                            if(directions_map.values().contains(i)){
                                i++;
                                logger_in_game.log("log", "input putting...");
                                continue;
                            }
                            directions_map.put(input,i);
                            logger_in_game.log("log", "input done");
                            break;
                        }
                        //
                    }
                });
                done.addActionListener(_ -> {deraction.this.setVisible(false);});
                this.addWindowListener(new WindowAdapter() {
                    @Override
                    public void windowClosing(WindowEvent e){
                        deraction.this.setVisible(false);
                    }
                });
                delete.addActionListener(_ -> {if(list.getSelectedIndex() != -1){list.remove(list.getSelectedIndex());}});
                buttons.add(add);
                buttons.add(delete);
                buttons.add(done);
                //done.addActionListener(_ -> {
                    /* creat_game creatGame =  *///new creat_game();
                    /* creatGame.pack();
                    creatGame.setVisible(true); */
                //});
                this.add(list, BorderLayout.CENTER);
                this.add(buttons, BorderLayout.NORTH);
                this.pack();
                //new Thread(synchronous).start();
                EventQueue.invokeLater(() -> new Thread(synchronous).start());
            }
            /* protected class adjust extends Thread{
                public void run() throws Exception{
                    for (String i : list.getItems()) {
                        if(!name.containsKey(i)){
                            int j = 0;
                            while(name.containsValue(j)){j++;}
                            name.put(i,j);
                        }
                    }
                    for (String i : name.keySet()) {
                        if(!Arrays.asList(list.getItems()).contains(i)){
                            //int j = 0;
                            //while(name.containsValue(j)){j++;}
                            //name.put(i,j);
                            //if(list.getItemCount() < name.get(i)){throw new Exception("list.getItemCount() < name.get(i)");}
                            //list.add(i,name.get(i));
                            name.remove(i);
                        }
                    }
                }
            } */
            protected class synchronous implements Runnable{
                @Override
                public void run() {
                    /* if (!check_order(directions_map)) {
                        //
                    } */
                    if(!check_order(directions_map)){
                        try {
                            ...
                            // TODO Auto-generated catch block
                            show_exception(e, deraction.this);
                        }
                    }
                    list.removeAll();
                    for (Entry<String,Integer> i : directions_map.entrySet()) {
                        list.add(i.getKey(),i.getValue());
                        logger_in_game.log("log", "list adding...");
                    }
                    logger_in_game.log("log", "list done");
                }
            }
            @SuppressWarnings("unused")
            protected boolean check_order(Map<String,Integer> map){
                return find_mininum_empty_index(map) == map.size()/* true */;
            }
            protected int find_mininum_empty_index(Map<String,Integer> map){
                for (int i = 0; i < map.size(); i++) {if(!kit.exist_key(map, i)){return /* false */i;}}
                return map.size();
            }
            protected void sort(Map<String,Integer> map) throws Exception{
                sort(map,find_mininum_empty_index(map));
            }
            protected void sort(Map<String,Integer> map, int index) throws Exception{
                for (int i = index; i < map.size(); i++) {
                    if (kit.exist_key(map, i)) {
                        if(i+1 == map.size()){/* re */throw new Exception("impassable1(我還沒想好這裡要填什麼)");}
                        if(/* kit.get_key(map, i).length == 0 */kit.exist_key(map, i)){map.replace(kit.get_unique_key(map, i+1), i);}
                        sort(map,i+1);
                    }
                }
            }
        }
        class kit_with_logger{
            //logger logger;
...
/*                 for (int i : entry.getValue()) {
            ...
            //if (digitals.length != locations.length) {throw new Exception("The number of digitals is not equals to locations'.");}
                ...
                logger_in_game.log("log", digitals.toString());
                //logger_in_game.log("log", String.valueOf(digitals.getAsInt()));
                logger_in_game.log("log", String.valueOf(digitals.hashCode()));
                for (int i = 0; locations/*/table*/ != 0; i++){//do
            //System.out.println(locations + "," + Arrays.toString(result.toArray()));
                ...
                //System.out.println(table);
                    ...
                    logger_in_game.log("log", String.valueOf(result));
                    logger_in_game.log("log", "before:" + result.toString());
                    logger_in_game.log("log", String.valueOf(locations));
                    logger_in_game.log("log", String.valueOf(i));
                    //logger_in_game.log("log", java.util.Arrays.toString(table));
                    ...
                    logger_in_game.log("log", String.valueOf(result));
                    logger_in_game.log("log", "after:" + result.toString());
                    logger_in_game.log("log", String.valueOf(locations));
                    logger_in_game.log("log", String.valueOf(locations%table));
                    locations /= table;
                }// ;//別/0
            ...
            @SuppressWarnings("ImplicitArrayToString")
            private String numbers_to_string(int[] input, char[] chars) throws Exception{
                StringBuilder string_builder = new StringBuilder();
                for(int i : input){
                    if(i >= chars.length){throw new Exception("The builder of empty has a problemn.");}
                    string_builder.append(chars[i]);
                }
                logger_in_game.log("log",String.valueOf(input)/*.toString()*/);
                return string_builder.reverse().toString();
            }
            private String get_a_unused_string(ArrayList<String> used, char[] chars) throws Exception{
                int empty_builder = 0;
                try{
                    while (used.contains(numbers_to_string(decimal_to_mixed(() -> chars.length, empty_builder), chars))) { 
                        empty_builder++;
                    }
                    logger_in_game.log("log",String.valueOf(empty_builder));
                    return numbers_to_string(decimal_to_mixed(() -> chars.length, empty_builder), chars);
                }catch(Exception e){
                    throw e;
                }
            }
            ...
            public /* static  */String[] get_key(Map<String,Integer> map, int value){
                return map.entrySet().stream()
                                     .filter(o -> o.getValue() == value)
                                     .map(Map.Entry::getKey)
                                     .toArray(String[]::new);
            }
            public String get_unique_key(Map<String,Integer> map, int value) throws Exception{
                if(get_key(map, value).length > 1){throw new Exception("n -> 1");}
                if(get_key(map, value).length == 0){return null;}
                return get_key(map, value)[0];
            }
            ...
            public kit_with_logger(logger logger) {
                this.logger = logger;
            } */
        }
        ...
        public String input(String title/* , Frame frame */,String text){
            //Frame frame = new Frame(/* frame, */title/* ,true */);
            Dialog frame = new Dialog((Frame) null, title/* , true */,true);
            ...
            ok.addActionListener(_ -> {/* b[0] = true; *//* b[0] = true; */result[0] = input.getText();frame.setVisible(false);});
...
/*public*/ class logger{
    ...
    public static class file_id_creater{
    ...
    public static class file_logger{
        ...
        private Path text_directory;
        Map<String, BufferedWriter> writers = new HashMap<>();
        boolean exit_if_fail;
        @SuppressWarnings("CallToPrintStackTrace")
        public file_logger(Path root_directory, String text_id, boolean exit_if_fail, String text) /*throws IOException*/{
            this.root_directory = root_directory;
            this.id = text_id;
            this.exit_if_fail = exit_if_fail;
            try{
                Files.createDirectories(root_directory);
                this.text_directory = file_util.creat_directory_name(this.root_directory, text + "-" + this.id);
            }catch(IOException e){
                e.printStackTrace();
                ...
                //return;
                //this.root_directory = root_directory;
                this.text_directory = null;
                if (exit_if_fail) {System.exit(0);}
            }
        }
        Path new_log(String category) throws IOException{
            if (text_directory == null) {
                throw new IllegalStateException("Logger not initialized: file_directory is null");
            }
            return file_util.creat_directory(text_directory, category, ".log");
        }
        @SuppressWarnings("CallToPrintStackTrace")
