...
public class tic_tac_toc {
    ...
    static private class show extends Frame{
        int directions_of_x;
        int directions_of_y;
        ...
        static int[] locations = new int[digital_of_directions.length];
        ...
        private void reflash(){
            ...
            for (int i = 0;i<board.length;i++) {
                for (int j = 0;j<board[i].length;j++){
                    int[] ij = {i,j};
                    try {
                        if(exist(units, digital_transforming(digital_of_directions,directions,ij,locations))){
        ...
        public show(int x,int y) throws HeadlessException {
            directions_of_x = x;
            directions_of_y = y;
        }
    }
    ...
    public static void main(String[] args) {
        directions_map.put("x", 0);
        directions_map.put("y", 1);
        directions_map.put("z", 2);
        total_numbers_of_directions = {2,2,2};
        try{
            show a = new show(0,1);
            a.setVisible(true);
        }catch(Exception _){}
