...
public class tic_tac_toc18 {
    static private class game{
                    ...
                    for (int j = 0;j<board[i].length;j++){
                        ...
                        try {
                            if(exist(units, digital_transforming(total_numbers_of_directions,directions,ij,locations))){
                                    //try{
                                button_board[i][j] = new Button(units[digital_transforming(total_numbers_of_directions,directions,ij,locations)].show);
                                final int fi,fj;
                                fi = i;
                                fj = j;
            ...
            public show(int x,int y,int number) throws HeadlessException {
                ...
                for (int i = 0; i < units.length; i++) {
                    try {
                        ...
                        units[i] = new unit(java.util.Arrays.toString(digital_to_array_transforming(total_numbers_of_directions, i)),i);
                    } catch (Exception e) {
                        Dialog d = new Dialog(this,e.getMessage(),true);
                        d.add(new Label(e.getMessage()));
                        d.setVisible(true);
        ...
        private boolean exist(unit[] units, int value){
            ...
            if(units == null || units[0] == null){return false;}
    ...
    public static void main(String[] args) {
        ...
        game.total_numbers_of_directions = new int[]{2,2,2};
        try{
