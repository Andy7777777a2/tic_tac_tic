    ...
    static private class game{
        ...
        private class game_main{
            ...
            private void initialize(String[] location, int[] total, unit[] edits, ArrayList<String> players){
                ...
                try{
                    while (game.this.players.contains(to_player(digital_to_array_transforming(new IntSupplier(){@Override public int getAsInt() {return EMPTY_BUIDER.length;}}, empty_builder)))) { 
                        empty_builder++;
                    }
                    empty = to_player(digital_to_array_transforming(new IntSupplier(){@Override public int getAsInt() {return EMPTY_BUIDER.length;}}, empty_builder));
                }catch(Exception e){
        ...
        @SuppressWarnings("ImplicitArrayToString")
        private int[] digital_to_array_transforming(IntSupplier digitals,int locations) throws Exception{
            //if (choosen_digitals.length != numbers.length) {throw new Exception("The number of choosen digitals is not equals to \"numbers\".");}
            //if (digitals.length != locations.length) {throw new Exception("The number of digitals is not equals to locations'.");}
            int table = 1;
            ArrayList<Integer> result = new ArrayList<>();
            System.out.println("in");
            for (int i = 0 ; locations/table == 0 ; i++) {
                System.out.println(i + "," + digitals.toString());
                table *= digitals.getAsInt();
                result.add(locations%table);
            }
            System.out.println(locations + "," + Arrays.toString(result.toArray()));
            return result.stream().mapToInt(i -> i).toArray();
        }
        private String to_player(int[] input) throws Exception{
